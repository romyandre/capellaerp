...
class Deliveryadvicedetail extends CActiveRecord
{
    ...
    public function beforeSave() {
      $this->reqdate = ($this->reqdate!=="")?date(Yii::app()->params['datetodb'], strtotime($this->reqdate)):new CDbExpression('NOW()');
	  $this->qty = Yii::app()->format->unformatNumber($this->qty);
	  $this->prqty=Yii::app()->format->unformatNumber($this->prqty);
		$this->giqty=Yii::app()->format->unformatNumber($this->giqty);
      return parent::beforeSave();
    }
	
		public function afterFind() {	
		$this->reqdate = date(Useraccess::model()->getformatdate(), strtotime($this->reqdate));
		$this->qty=Yii::app()->format->formatNumber($this->qty);
		$this->prqty=Yii::app()->format->formatNumber($this->prqty);
		$this->giqty=Yii::app()->format->formatNumber($this->giqty);
	}
...
$criteria->condition='t.deliveryadviceid='.$_GET['Deliveryadvicedetail']['deliveryadviceid'];
} else {
