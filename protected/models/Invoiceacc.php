...
class Invoiceacc extends CActiveRecord
{
	...
	public function getTotaldebit()
	{
		...
		$connection=Yii::app()->db;
			  try
			  {
				$sql = 'select sum(ifnull(debit,0)) as total from invoiceacc where invoiceid= '.$this->invoiceid;
		...
		public function getTotalcredit()
	{
		...
		$connection=Yii::app()->db;
			  try
			  {
				$sql = 'select sum(ifnull(credit,0)) as total from invoiceacc where invoiceid= '.$this->invoiceid;
				$command=$connection->createCommand($sql);
				$row = 	$command->queryRow();
				$total = $row['total'];
				return $total;
