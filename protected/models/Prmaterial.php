...
$criteria->condition='t.prheaderid='.$_GET['Prmaterial']['prheaderid'];
} else {
$criteria->condition='t.prheaderid=0';
        ...
        public function searchwfstatus()
	{
		...
		$criteria=new CDbCriteria;
                $criteria->with=array('prheader','product');
...
inner join useraccess e on e.useraccessid = d.useraccessid
where upper(a.wfname) = upper('listpr') and upper(e.username)=upper('".Yii::app()->user->name."') 
and prheader.prno is not null
and t.prmaterialid not in 
(
select zz.prdetailid
from podetail zz 
where zz.productid = t.productid and zz.prdetailid = t.prmaterialid
and zz.poqty <= t.qty
)
)";
		$criteria->compare('prmaterialid',$this->prmaterialid);
