...
{
	$criteria->compare('productname',$_GET['productname'],true);
}
}
	/**
...
'pagination'=>array(
	...
	public function searchslocstatus()
	{
		// Warning: Please modify the following code to remove attributes that
		// should not be searched.
		...
		if (isset($_GET['slocid']))
		{
			$criteria->condition = " t.recordstatus = 1 and  
				t.productid in (select productid from productplant 
				where slocid = ".$_GET['slocid'].")";
		}
		$criteria->compare('productid',$this->productid);
		$criteria->compare('productname',$this->productname,true);
		$criteria->compare('recordstatus',$this->recordstatus,true);
